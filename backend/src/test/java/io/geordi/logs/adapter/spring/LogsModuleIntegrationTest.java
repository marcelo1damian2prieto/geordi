...
@SpringBootTest(
        ...
        properties = {"geordi.modules.logs.enabled=false", "geordi.modules.metrics.enabled=false",
            "geordi.modules.traces.enabled=false", "geordi.modules.service-map.enabled=false"},
        webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
class LogsModuleIntegrationTest {
    ...
    void disabledModuleRemainsInInventorySkipsHealthAndHasNoRoutes() {
        ...
        assertThat(modules).filteredOn(item -> item.get("id").equals("logs"))
        ...
        assertThat(healthModules).filteredOn(item -> item.get("id").equals("logs"))
                .singleElement().satisfies(item -> assertThat(item).containsEntry("status", "DISABLED"));

        assertThat(restTemplate.getForEntity(url("/api/logs/services"), String.class).getStatusCode())
