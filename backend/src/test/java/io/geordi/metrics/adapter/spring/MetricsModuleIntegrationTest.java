...
class MetricsModuleIntegrationTest {
    ...
    void disabledModuleRemainsInInventorySkipsHealthAndHasNoRoutes() {
        ...
        assertThat(modules).filteredOn(item -> item.get("id").equals("metrics"))
        ...
        assertThat(healthModules).filteredOn(item -> item.get("id").equals("metrics"))
                .singleElement().satisfies(item -> assertThat(item).containsEntry("status", "DISABLED"));

        assertThat(restTemplate.getForEntity(url("/api/metrics/services"), String.class).getStatusCode())
